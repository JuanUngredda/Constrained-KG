...
class gradients(object):
    def __init__(self, x_new=None, model=None, xopt =None, Z=None, aux=None, aux2=None, varX=None, dvar_dX=None, test_samples= None):
        self.x_new = x_new
        self.model = model
        self.Z = Z
        self.aux = aux
    ...
    def compute_value_mu_xnew(self, x):
        ...
        cov = self.model.posterior_covariance_between_points_partially_precomputed(x, self.x_new)[:, :, 0]
        func_val = []
    ...
    def compute_gradient_mu_xnew(self,x ):
        ...
        dcov_dX_inner = self.model.posterior_covariance_gradient_partially_precomputed(x, self.x_new)
        cov = self.model.posterior_covariance_between_points_partially_precomputed(x, self.x_new)[:, :, 0]
    ...
    def compute_posterior_var_x_new(self, x, likelihood=False):
        ...
        cov = self.model.posterior_covariance_between_points(x, self.x_new)[:, :, 0]
        kernel_current = self.model.posterior_covariance_between_points(x,x)
        func_val = []
        for j in range(cov.shape[0]):
            b = np.sqrt(self.aux[j] * np.square(cov[j]))
            # if not likelihood:
            #     likelihood_noise = self.model.get_model_parameters()[j][:,-1]
            #     kernel_current_lik = kernel_current[j] + np.identity(kernel_current[j].shape[0])*likelihood_noise
            # else:
            kernel_current_lik = kernel_current#[j]
            b = b[np.newaxis]
    ...
    def compute_gradient_posterior_var_x_new(self, x):
        ...
        K_x_xnew = self.model.posterior_covariance_between_points_partially_precomputed(x, self.x_new)[:, :, 0]
        grad_K_x_xnew = self.model.posterior_covariance_gradient_partially_precomputed(x, self.x_new)[:, :, 0]
    ...
    # def compute_gradient_posterior_std(self, x):
    #
    #     posterior_var = self.model.posterior_variance(x)
    #     gradient_posterior_var = self.mode.posterior_variance_gradient
    #     gradient_posterior_std_x_new = (1.0 / 2) * (posterior_var) ** (- 1.0 / 2) * gradient_posterior_var
    #     return gradient_posterior_std_x_new

    def compute_probability_feasibility_multi_gp(self, x, l=0):
    ...
    def compute_gradient_probability_feasibility(self, x, model, mean=None, var=None):
        ...
        if (mean is None) or (var is None):
            ...
            var = model.posterior_covariance_between_points_partially_precomputed(x, x)[:, :, 0]
    ...
    def compute_KG_xopt(self,x_new):
        self.xnew = x_new
        self.varX = self.model.posterior_variance(self.xnew)[:,0]
        self.dvar_dX = self.model.posterior_variance_gradient(self.xnew)[:,0,:]
        self.aux = np.reciprocal(self.varX)
        self.aux2 = np.square(np.reciprocal(self.varX))
        out  = self.compute_value_mu_xnew(self.xopt)
        return out

    def compute_grad_probability_feasibility_multi_gp_xopt(self, xopt, l=0):
        mean = self.compute_value_mu_xnew(x=xopt)
        cov = self.compute_posterior_var_x_new( x=xopt , likelihood=True)
        Fz = []
        grad_Fz = []
        for m in range(self.model.output_dim):
    ...
    def compute_gradient_probability_feasibility_xopt(self,xopt , model, mean=None, var=None):
        ...
        if (mean is None) or (var is None):
            mean = model.posterior_mean(xopt)
            var = model.posterior_covariance_between_points_partially_precomputed(xopt, xopt)[:, :, 0]
        std = np.sqrt(var).reshape(-1, 1)
        ...
        print("grad_mean",grad_mean,"grad_std ",grad_std )
        for d in dims:
            grd_mean_d = grad_mean[:, d].reshape(-1, 1)
            grd_std_d = grad_std[:, d].reshape(-1, 1)
            grad_func_val = -fz * aux_var * (grd_mean_d * std - mean * grd_std_d )
    ...
    def compute_grad_mu_xopt(self, xopt):
        ...
        cov_opt = self.model.posterior_covariance_between_points_partially_precomputed(xopt, self.x_new)[:, 0, 0]
        dcov_opt_dx = self.model.posterior_covariance_gradient_partially_precomputed(self.x_new, xopt)[:, 0, :]
    ...
    def compute_b(self,xopt, x_new = None):
        if x_new is None:
            ...
            cov_opt = self.model.posterior_covariance_between_points_partially_precomputed(xopt, x_new)[:, 0, 0]
            b = np.sqrt(np.matmul(self.aux,np.square(cov_opt)))
    ...
    def compute_grad_sigma_xopt(self, xopt):
        ...
        posterior_var_xopt = self.compute_posterior_var_x_new(xopt)
    ...
    def trial_compute_probability_feasibility_multi_gp(self, xnew):
        self.xnew = xnew
        self.varX= self.model.posterior_variance(xnew)[:, 0]
        self.dvar_dX = self.model.posterior_variance_gradient(xnew)[:,0,:]
        self.aux = np.reciprocal(self.varX)
        self.aux2 = np.square(np.reciprocal(self.varX))
        return self.compute_probability_feasibility_multi_gp(self.xopt)
    ...
    def test_mode(self):
        ...
        f_9 = [self.trial_compute_probability_feasibility_multi_gp, self.trial_compute_grad_probability_feasibility_multi_gp_xopt]
        ...
        for index in range(len(f)):
            ...
            #self.gradient_sanity_check_1D(name[index], f[index][0], f[index][1])
    ...
    def gradient_cov_check(self, delta=1e-10):
...
#            print("FD", np.array(f_delta / (2 * delta)).reshape(-1), "analytical", grad_f(x).reshape(-1))
    ...
    def gradient_sanity_check_1D(self, name, f, grad_f, delta= 1e-10):
        X = self.test_samples
        numerical_grad = []
        analytical_grad = []
        func_val = []
        ...
        ax1.scatter(X, self.mean_plot,label="mean")
        ax1.legend()
        ax2.scatter(X, self.var_plot, label="var")
        ax2.legend()
