...
import GPyOpt
from GPyOpt.objective_examples.experiments2d import mistery, test_function_2
...
def function_caller_test_func_2(it):
    for rep in expdict[it]:
        np.random.seed(rep)
        for noise in [1e-06]:
            for num_samples in [50, 500, 50000, 500000]:
                # func2 = dropwave()
                ...
                # # --- Initial design
                # initial design
                underlying_discrete_space = GPyOpt.experiment_design.initial_design('latin', space,
                                                                                    num_underlying_samples)
...
# for i in range(30):
# function_caller_test_func_2(it=1)
