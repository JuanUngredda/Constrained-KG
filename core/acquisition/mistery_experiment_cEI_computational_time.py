...
def function_caller_mistery_cEI(rep):
    ...
    for noise in [1.0]:
        ...
        max_iter  = 100
        stop_date = datetime(2023, 5, 9, 7)  # year month day hour
        # print("Finished Initialization")
        subfolder = "mistery_cEI_n_obj_" + str(noise_objective) + "_n_c_" + str(noise_constraints)
        folder = "RESULTS"
        cwd = os.getcwd()
        path =cwd + "/" + folder + "/" + subfolder + '/it_' + str(rep) + '.csv'
        X, Y, C, recommended_val, optimum, Opportunity_cost = bo.run_optimization(max_iter = 1,verbosity=False,
                                                                                  stop_date = stop_date,
...
for r in range(100):
    function_caller_mistery_cEI(rep=r)
