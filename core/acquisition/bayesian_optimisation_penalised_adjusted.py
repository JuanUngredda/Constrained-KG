...
class BO(object):
    ...
    def optimize_final_evaluation(self, KG_dynamic_optimisation):
        ...
        aggregated_posterior = self.wrapped_agreagated_penalised_posterior(f = self.agreagated_penalised_posterior)
        out = self.acquisition.optimizer.optimize(f=aggregated_posterior, duplicate_manager=None,
    ...
    def true_best_value(self):
        ...
        X = initial_design('random', self.space, 100)
    ...
    def func_val(self, x):
        ...
        agregated_constraints = np.product(np.concatenate(C, axis=1) < 0, axis=1).reshape(-1)
        agregated_constraints[agregated_constraints == 0] = 9999
        out = Y.reshape(-1)* agregated_constraints
