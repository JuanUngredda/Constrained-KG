    ...
    def run_optimization(self, max_iter = 1,
        ...
        while (self.max_iter > self.num_acquisitions ) and (self.stop_date > today):
            ...
            if verbosity:
                ...
                initial_design = GPyOpt.experiment_design.initial_design('latin', self.space, 100)
                fvals, _ = self.objective.evaluate(initial_design)

                fvals = np.array(fvals).reshape(-1)

                plt.title("real surface")
                plt.scatter(initial_design, fvals)
                plt.scatter(self.X, self.Y, color="magenta")
                plt.show()

                # acq_vals = self.acquisition._compute_acq(initial_design)

                acq_vals = self.acquisition._compute_acq(np.array([6.4]))


                plt.scatter(initial_design, acq_vals)
                plt.show()
