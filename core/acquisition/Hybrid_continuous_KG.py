...
class KG(AcquisitionBase):
    ...
    def _plots(self, test_samples, discretisation=None):
        ...
        if False:
            ...
            # plt.savefig(
            #     "/home/juan/Documents/repos_data/Constrained-KG/RESULTS/plot_saved_data/plots/posterior_GP_{n}.jpg".format(n = len(self.model.get_X_values())),
            #     bbox_inches="tight")
            plt.show()
            ...
            # plt.legend(loc="upper right", prop={'size': 14})
            # plt.savefig(
            #     "/home/juan/Documents/repos_data/Constrained-KG/RESULTS/plot_saved_data/plots/cKG_{n}.jpg".format(n = len(self.model.get_X_values())),
            #     bbox_inches="tight")
            plt.show()
    ...
    def discrete_KG(self, Xd, xnew, Zc, aux_obj, aux_c, grad=False, verbose=False):
        ...
        for zc in range(Zc.shape[0]):
            marginal_KG.append(self.parallel_KG(MM=MM*np.array(Fz[:,zc]).reshape(-1),SS=SS*np.array(Fz[:,zc]).reshape(-1),verbose=verbose))
