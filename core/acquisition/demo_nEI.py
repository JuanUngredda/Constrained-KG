...
def generate_initial_data(n=10):
    ...
    lb = bounds[0, :]
    delta = ub - lb
    train_x = torch.rand(10, 2, device=device, dtype=dtype) * delta + lb
...
BATCH_SIZE = 1
bounds = torch.tensor([[0.0] * 2, [5.0] * 2], device=device, dtype=dtype)
...
for trial in range(1, N_TRIALS + 1):
    ...
    for iteration in range(1, N_BATCH + 1):
        ...
        qEI = qExpectedImprovement(
            model=model_nei,
            best_f=(train_obj_nei * (train_con_nei <= 0).to(train_obj_nei)).max(),
            sampler=qmc_sampler,
        ...
        print("new_x_ei, new_obj_ei, new_con_ei",new_x_ei, new_obj_ei, new_con_ei)
