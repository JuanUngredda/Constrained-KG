...
class KG(AcquisitionBase):
    ...
    def _compute_acq(self, X):
        ...
        if self.number_of_evaluations != Number_X_evaluations:
            ...
            # self.number_of_evaluations = Number_X_evaluations
            #
            # plot_samples = initial_design('random', self.space, 1000)
            # mu = self.model.posterior_mean(plot_samples)[0]
            #
            # plt.title("unconstrained surface and worst value")
            # plt.scatter(plot_samples[:,0], plot_samples[:,1], c=mu )
            # plt.scatter(_[:, 0], _[:, 1] , color="red" )
            # plt.show()
            #
            # mu_constrained = -self.current_func(plot_samples)
            # plt.title("constrained surface and best value")
            # plt.scatter(self.current_max_xopt[:,0], self.current_max_xopt[:,1], color="red")
            # plt.scatter(plot_samples[:,0],plot_samples[:,1], c=mu_constrained.reshape(-1))
            # plt.show()
            #
            # print("penalty M", self.M)
            # print("best constrained value", self.current_max_value.reshape(-1))
            # raise
    ...
    def current_func(self, X_inner):
        ...
        feasable_term = mu * Fz
        infeasible_term = self.M.squeeze() * (1 - Fz)
    ...
    def update_penalty(self):
        inner_opt_x, inner_opt_val = self.optimizer.optimize_inner_func(f=self.current_func_no_constraint,
                                                                        f_df=None,
                                                                        num_samples=50)
